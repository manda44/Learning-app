import {Grid, Paper, Button, Center, Box, Checkbox, Group, Textarea, Text, Divider, ActionIcon} from "@mantine/core";
import {BlockNoteView} from "@blocknote/mantine";
import {useCreateBlockNote} from "@blocknote/react";
import {fr} from "@blocknote/core/locales";
import {codeBlock} from "@blocknote/code-block";
import {IconPlus, IconQuestionMark, IconList, IconMessageCircle, IconTrash, IconCopy, IconChevronUp, IconChevronDown} from "@tabler/icons-react";
...
const Mcq = forwardRef<McqRef, McqProps>((props, ref) => {
            ...
            if (validateData()) {
                return ({
                    ...
                    questionItems: optionEditors.slice(0, optionCount).map((option, index) => ({
                        ...
                        isRight: form.values.options[index]?.checked || false,
    ...
    const optionEditors: McqOption[] = useMemo(() => [
    ...
    useEffect(() => {
        if (props.data?.questionItems) {
            ...
            form.setValues({ options: formOptions });
            ...
            // Reset errors after loading data
            setErrors({});
        }
    }, []);
                ...
                <form onSubmit={form.onSubmit((values) => console.log(values))}>
                    ...
                    <Box p={20}>
                        <Grid>
                            <Grid.Col span={12}>
                                <Group align="center" mb="md">
                                    <IconList size={24} color="#28a745" />
                                    <Text size="xl" fw={600} c="#28a745">
                                        Réponses
                                    </Text>
                                ...
                                <Box style={{ minHeight: '5vh' }}>
                                    {optionEditors.slice(0, optionCount).map((option, index) => (
                                        <Group key={option.id} justify="flex-start" align="center" mb="sm">
                                            <Checkbox
                                                {...form.getInputProps(`options.${index}.checked`, { type: 'checkbox' })}
                                                mr="md"
                                                size="md"
                                            ...
                                            <Box flex={1} style={{
                                                ...
                                                borderLeft: errors.options?.[index] ? '4px solid #ff6b6b' : '4px solid #28a745',
                                                borderRadius: '4px'
                                            }} pt={"md"}>
                               ...
                               <Center mt={20}>
                                    <Button
                                        ...
                                        onClick={() => {
                                            addOption();
                                            setErrors({}); // Reset errors when adding option
                                        }}
