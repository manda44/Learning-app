...
import { getMiniProjectsByCourse } from '../services/miniProjectService';
import { MiniProjectItem } from '../components/MiniProjectItem';
...
export function CourseView() {
  ...
  const [miniProjects, setMiniProjects] = useState<MiniProjectBasic[]>([]);
  ...
  const fetchMiniProjects = useCallback(async () => {
    try {
      if (!courseId) return;
      const projects = await getMiniProjectsByCourse(parseInt(courseId));
      setMiniProjects(projects);
    } catch (err) {
            ...
            <ScrollArea style={{ flex: 1 }}>
              <Stack gap="xs" pr="lg">
                ...
                {miniProjects.map((project) => (
                  <MiniProjectItem key={`miniproject-${project.miniProjectId}`} miniProject={project} />
                ))}
              </Stack>
            </ScrollArea>
          </Paper>
