-- ============================================================================
-- INSERT STUDENT COURSE ENROLLMENTS FROM enrollments.csv (DEDUPLICATED)
-- Maps students to courses with progress and completion data
-- Automatically removes duplicate studentId-courseId pairs
-- ============================================================================

-- ============================================================================
-- STEP 1: Create temporary table to store raw CSV data
-- ============================================================================

IF OBJECT_ID('tempdb..#EnrollmentData') IS NOT NULL
...
INSERT INTO #EnrollmentData (student_id, course_id, progress_percentage, completed_at)
VALUES
(816,16,89,NULL),
(176,16,64,NULL),
(85,8,52,NULL),
...
-- ============================================================================
-- STEP 3: Deduplicate and identify unique enrollments
-- ============================================================================

-- Get a count of total records and duplicates
PRINT '===================================================='
...
SELECT
    ...
    COUNT(DISTINCT course_id) as [Unique Courses]
FROM #EnrollmentData;

-- Check for duplicates
DECLARE @DuplicateCount INT
SELECT @DuplicateCount = COUNT(*) - COUNT(DISTINCT CONCAT(student_id, '-', course_id))
FROM #EnrollmentData;

PRINT 'Duplicate Entries: ' + CAST(@DuplicateCount AS VARCHAR(10))
PRINT '===================================================='

-- ============================================================================
-- STEP 4: Insert only unique studentId-courseId combinations into production
-- ============================================================================

PRINT ''
PRINT 'Starting enrollment insertion...'

-- Using CTE to select only the first occurrence of each student-course pair
;WITH UniqueEnrollments AS (
    SELECT
        ...
        progress_percentage,
        completed_at,
        ROW_NUMBER() OVER (PARTITION BY student_id, course_id ORDER BY progress_percentage DESC, completed_at DESC) as rn
    FROM #EnrollmentData
)
...
SELECT
    ue.student_id AS [StudentId],
    ue.course_id AS [CourseId],
    CASE
        -- If completed_at is not null, enrollment date is 30 days before completion
        WHEN ue.completed_at IS NOT NULL THEN DATEADD(DAY, -30, CAST(ue.completed_at AS DATE))
    ...
    CASE
        ...
        WHEN ue.progress_percentage = 100 AND ue.completed_at IS NOT NULL THEN 'completed'
        -- If progress is 100% but no completion date, also completed
        WHEN ue.progress_percentage = 100 THEN 'completed'
        -- If progress is 0%, status is 'dropped'
        WHEN ue.progress_percentage = 0 THEN 'dropped'
        -- Otherwise status is 'active'
        ELSE 'active'
    END AS [Status],
    ue.progress_percentage AS [ProgressPercentage],
    CASE
        -- If completed_at is provided, cast it as DATE; otherwise NULL
        WHEN ue.completed_at IS NOT NULL THEN CAST(ue.completed_at AS DATE)
        ELSE NULL
    END AS [CompletionDate]
FROM UniqueEnrollments ue
WHERE ue.rn = 1  -- Only keep the first occurrence of each student-course pair
AND NOT EXISTS (
    -- Avoid duplicate enrollments in the actual table
    SELECT 1 FROM [dbo].[StudentCourseEnrollment]
    WHERE [StudentId] = ue.student_id AND [CourseId] = ue.course_id
);
...
PRINT ''
PRINT 'ENROLLMENT INSERTION COMPLETED'
PRINT '===================================================='
...
SELECT
    ...
    CAST(AVG(CAST([ProgressPercentage] AS FLOAT)) AS DECIMAL(5,2)) as [Avg Progress %]
...
PRINT ''
PRINT 'Sample Completed Enrollments:'
SELECT TOP 10
    e.[EnrollmentId],
    e.[StudentId],
